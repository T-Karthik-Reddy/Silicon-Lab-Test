...
# ==========================================
# HARDWARE-READY DSP FUNCTIONS
# (Same as inference.py to ensure consistency)
# ==========================================
...
# ==========================================
# INT8 INFERENCE CLASS
# ==========================================

class CryDetectorInt8:
    def __init__(self, tflite_path, stats_path=None):
        """Initialize Int8 Cry Detector"""
        
        if not os.path.exists(tflite_path):
            raise FileNotFoundError(f"TFLite model not found: {tflite_path}")

        print(f"Loading INT8 Model: {os.path.basename(tflite_path)}")
        ...
        # 3. Get Quantization Parameters (CRITICAL FOR INT8)
        # These values tell us how to convert Float -> Int8
        ...
        # Verify it is actually an Int8 model
        if self.input_details['dtype'] != np.int8:
            print("⚠️ WARNING: This model expects " + str(self.input_details['dtype']) + ", but this script is for Int8.")
        
        print(f"✓ Quantization Parameters:")
        print(f"  Input  Scale: {self.input_scale:.6f}, Zero Point: {self.input_zero_point}")
        print(f"  Output Scale: {self.output_scale:.6f}, Zero Point: {self.output_zero_point}")
        ...
        if os.path.exists(stats_path):
            ...
            print("! Warning: Using default stats")

        self.mel_basis = create_mel_filterbank(config.SAMPLE_RATE, config.N_FFT, config.N_MELS, 50, 8000)
    ...
    def predict(self, audio_path, visualize=False, save_viz=None):
        # 1. Preprocess (Result is Float32)
        mel_spec, duration, audio = self.preprocess_audio(audio_path)
        if mel_spec is None: return {'error': 'Empty file'}

        # 2. Normalize (Result is Float32)
        mel_spec_norm = (mel_spec - self.mean) / (self.std + 1e-8)
        
        # 3. Reshape (Add Batch/Channel dims)
        ...
        if tuple(self.input_shape) != input_data.shape:
            # Try transposing if dims mismatch
            input_data_T = np.transpose(input_data, (0, 1, 3, 2))
            if tuple(self.input_shape) == input_data_T.shape:
                input_data = input_data_T

        # ==========================================================
        # 4. QUANTIZATION (Float32 -> Int8)
        # This is the math you must implement in C on the chip
        # Formula: q = (real_value / scale) + zero_point
        # ==========================================================
        
        # Apply formula
        input_int8 = (input_data / self.input_scale) + self.input_zero_point
        
        # Clip to Int8 Range [-128, 127]
        input_int8 = np.clip(input_int8, -128, 127)
        
        # Cast to integer type
        input_int8 = input_int8.astype(np.int8)

        # 5. Run Inference
        self.interpreter.set_tensor(self.input_index, input_int8)
        self.interpreter.invoke()
        output_int8 = self.interpreter.get_tensor(self.output_index)

        # ==========================================================
        # 6. DEQUANTIZATION (Int8 -> Float32)
        # Formula: real_value = (q - zero_point) * scale
        # ==========================================================
        output_float = (output_int8.astype(np.float32) - self.output_zero_point) * self.output_scale

        # 7. Softmax (if output is raw logits)
        probs = tf.nn.softmax(output_float, axis=1).numpy()
    ...
    def _visualize(self, audio, mel_spec, result, save_path=None):
        ...
        axes[1].set_title('Mel Spectrogram (Int8 Input)')
        
        color = 'red' if result['is_crying'] else 'green'
        info = f"INT8 PREDICTION\n{result['predicted_class'].upper()}\nConf: {result['confidence']:.1%}"
        fig.text(0.5, 0.02, info, ha='center', fontsize=12, bbox=dict(facecolor=color, alpha=0.3))
...
def main():
    import argparse
    # Default to Int8 model
    default_model = "audio_model_int8.tflite"
    parser = argparse.ArgumentParser(description='Int8 TFLite Inference')
    parser.add_argument('audio_files', nargs='+', help='Path to audio files')
    parser.add_argument('--model', default=default_model, help='Path to model_int8.tflite')
    ...
    try:
        detector = CryDetectorInt8(args.model)
        if args.output: os.makedirs(args.output, exist_ok=True)
        
        print("\n" + "="*50)
        print("Int8 Inference Started")
        print("="*50)
        
        for f in args.audio_files:
            save_path = None
            if args.output:
                save_path = os.path.join(args.output, Path(f).stem + "_int8.png")
